...
function vehicleShader_webgl2() {
    return new Program("VehicleShader", {
        uniforms: {
            viewMatrix: 'mat4',
        ...
        fragmentShader:
            `#version 300 es
                ...
                void main(void) {
                    ...
                    outScreen = vec4(lightWeighting, 1.0) * color;
                }`
    });
};

const MODEL_FORWARD = new Vec3(0, 0, -1);
...
class Planemarker {
    constructor(options) {
        ...
        this._lonLatAlt = new LonLat(0, 0, 3000);

        this.pitch = options.pitch || 0.0;
        this.yaw = options.ya || 0.0;
        ...
        this._posHigh = new Float32Array(3);
        this._posLow = new Float32Array(3);
        ...
        this._color = new Float32Array([1.0, 1.0, 0.0, 1.0]);

        this.scale = 0.02;
        this.scaleByDistance = new Float32Array(options.scaleByDistance || [math.MAX32, math.MAX32, math.MAX32]);
        this._viewScale = 1.0;
    }

    set(lon, lat, alt, pitch, yaw, roll) {
    ...
    update() {
        ...
        this._scene._planet.ellipsoid.lonLatToCartesianRes(this._lonLatAlt, this._posCart);
        this._qNorthFrame = getNorthBearingRotationFrame(this._posCart);
    ...
    init() {
        ...
        this._scene.renderer.handler.addProgram(vehicleShader_webgl2());

        let v = new Object3d(models.unknown());

        this.verticesBuffer = this._scene.renderer.handler.createArrayBuffer(new Float32Array(v.vertices), 3, v.vertices.length / 3);
        this.normalBuffer = this._scene.renderer.handler.createArrayBuffer(new Float32Array(v.normals), 3, v.normals.length / 3);
    ...
    draw() {
        ...
        var sh = r.handler.programs.VehicleShader;
        var p = sh._program;
        var gl = r.handler.gl;
        ...
        gl.uniform4fv(u.lightsPositions, this._scene._planet._lightsTransformedPositions);
        gl.uniform3fv(u.lightsParamsv, this._scene._planet._lightsParamsv);
        gl.uniform1fv(u.lightsParamsf, this._scene._planet._lightsParamsf);
        ...
        gl.uniform2fv(u.pitchRoll, [-this.pitch, this.roll]);
        gl.uniform4fv(u.color, this._color);
        gl.uniform3fv(u.direction, this._uOrientation);
        //

        gl.bindBuffer(gl.ARRAY_BUFFER, this.normalBuffer);
        gl.vertexAttribPointer(a.aVertexNormal, this.normalBuffer.itemSize, gl.FLOAT, false, 0, 0);

        gl.bindBuffer(gl.ARRAY_BUFFER, this.verticesBuffer);
        gl.vertexAttribPointer(a.aVertexPosition, this.verticesBuffer.itemSize, gl.FLOAT, false, 0, 0);

        gl.drawArrays(gl.TRIANGLES, 0, this.verticesBuffer.numItems);
    }
}

class TrafficScene extends RenderNode {
    constructor(options) {
        super("SkyTraffic");
...
var s = new TrafficScene();

s.bind(globe.planet);

globe.planet.renderer.addNode(s);

window.trafficScene = s;
window.Vec3 = Vec3;
